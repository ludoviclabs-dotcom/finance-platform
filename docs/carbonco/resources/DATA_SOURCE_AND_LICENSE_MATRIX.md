...
> **Phase :** cadrage (lecture seule). **Date de vérification de toutes les lignes : 2026-07-22** (heure locale Europe/Paris ; les commits Git de cette branche, horodatés en **UTC**, lisent 2026-07-21 en fin de journée — même moment calendaire, pas une date future).
...
- **4 Eurostat** — V / V / V / V / **V\*** / V (⚠ certaines données pays hors UE exclues) / V / « Source: Eurostat » · **\*** usage dérivé OK pour les séries CC BY sans réserve ; le sous-ensemble **pays hors UE / commerce CH-AT** porte une réserve commerciale que le booléen n'exécute pas → **gate requis** (garder `derived_use_allowed=false` sur ce sous-ensemble tant que le modèle n'est pas étendu, cf. §6 / O-10)
- **5 UN Comtrade** — V / V (quotas) / V / conditionnel / V (transformé) / restreint / **F (brut)** / « Source: UN Comtrade (© United Nations) »
- **6 FAOSTAT** — V / V / V / V / **V\*** / **conditionnel** (pas de promotion d'entreprise commerciale / endossement) / V / « FAO. [année]. [db: dataset]. Licence: CC BY 4.0 » · **\*** usage dérivé CC BY OK en principe, mais la réserve « pas d'usage commercial promotionnel » n'est **pas** portée par le booléen (`allow_derived_use` ignore `commercial_use_allowed`) → **gate requis** avant tout usage dérivé commercial (garder `derived_use_allowed=false` tant que le modèle n'est pas étendu, cf. §6 / O-10)
...
- **FAOSTAT** et **Eurostat** portent des réserves commerciales **conditionnelles** (FAO : pas de promotion d'entreprise commerciale ni implication d'aval ; Eurostat : restriction commerciale sur certaines séries pays hors UE + commerce CH/AT). ⚠️ **Piège du modèle de licence — ne PAS poser `derived_use_allowed=true` en s'appuyant sur une réserve « conditionnelle ».** `license_policy.evaluate()` calcule `allow_derived_use = active AND derived_use_allowed` **sans consulter `commercial_use_allowed`** (`services/intelligence/license_policy.py` : `commercial_use_allowed=false` ne produit qu'un *warning*, pas un blocage). Régler `derived_use_allowed=true` tout en marquant la réserve commerciale « conditionnelle » **autoriserait donc un calcul dérivé commercial interdit** — et CarbonCo est un produit commercial, la condition ne peut PAS être portée par le booléen actuel. **Défaut sûr à l'onboarding de ces sources :** garder `derived_use_allowed=false` (ou n'ingérer que le sous-ensemble sans réserve, ex. séries UE d'Eurostat) **tant que** le modèle de licence n'a pas été étendu pour bloquer l'usage dérivé en contexte commercial quand `commercial_use_allowed=false`. Élément d'architecture — cf. `MODULE2_DECISIONS.md` §3 (O-10).
- Les sources ouvertes UE (3, 4, 9) se résolvent toutes en CC BY 4.0 / réutilisation UE → motif d'attribution commun possible — **sous réserve** du point ci-dessus pour le sous-ensemble Eurostat à restriction commerciale.
