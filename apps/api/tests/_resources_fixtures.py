...
def build_resources_db(conn) -> None:
    """DDL historique + 001-042 (Evidence Kernel 028, fondation catalogue 042)."""
...
def resources_schema():
    """Applique le schéma jusqu'à 042 une fois par module de test."""
    with get_db() as conn:
...
def two_companies_resources(resources_schema):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            for table in (*RESOURCES_TABLES, *EK_TABLES):
