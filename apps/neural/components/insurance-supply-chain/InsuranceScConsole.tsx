...
import {
  INSURANCE_SC_AGENTS,
  INSURANCE_SC_SCENARIOS,
  type InsuranceScAgentSlug,
  type InsuranceScGateState,
  type InsuranceScVerdict,
} from "@/lib/data/insurance-supply-chain-catalog";

const VERDICT_STYLE: Record<InsuranceScVerdict, string> = {
...
const GATE_STYLE: Record<InsuranceScGateState, string> = {
...
export function InsuranceScConsole() {
  const [activeAgent, setActiveAgent] = useState<InsuranceScAgentSlug>("repair-network-insur");
  const scenarios = useMemo(
    () => INSURANCE_SC_SCENARIOS.filter((scenario) => scenario.agentSlug === activeAgent),
    [activeAgent],
  );
  const [activeScenarioId, setActiveScenarioId] = useState(scenarios[0]?.id ?? "");

  const activeScenario = scenarios.find((scenario) => scenario.id === activeScenarioId) ?? scenarios[0];
  const activeAgentMeta = INSURANCE_SC_AGENTS.find((agent) => agent.slug === activeAgent);

  function handleAgentChange(slug: InsuranceScAgentSlug) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
      <div className="rounded-[20px] border border-white/10 bg-white/[0.045] p-3">
        <div className="px-2 pb-3 pt-1">
          ...
          <p className="mt-2 text-sm leading-relaxed text-white/62">
            12 cas synthetiques issus des workbooks. Les gates priment sur tout texte genere.
          </p>
        </div>
      ...
      <div className="rounded-[20px] border border-white/10 bg-[#091310] p-5 shadow-2xl shadow-black/20">
            ...
            <button
              ...
              className={[
                ...
                scenario.id === activeScenario.id
                  ? "border-white/35 bg-white text-[#07110f]"
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="font-mono text-[11px] uppercase tracking-[0.08em]">{gate.id}</p>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
            ...
            <div className="mt-4 space-y-3">
              {[
                ...
                ["HITL", activeScenario.verdict === "PASS" ? "selon gate" : "obligatoire"],
                ["Trace", "hash workbook"],
              ].map(([label, value]) => (
            ...
            <div className="mt-5 rounded-xl border border-cyan-300/20 bg-cyan-300/[0.08] p-3">
              <div className="flex items-center gap-2 text-cyan-100">
                ...
                <span className="text-xs font-semibold uppercase tracking-[0.12em]">MVP guardrail</span>
              </div>
              <p className="mt-2 text-xs leading-relaxed text-cyan-100/70">
                Aucun refus de sinistre, aucune sanction fournisseur et aucune decision sensible ne sort sans revue humaine.
              </p>
            </div>
          </div>
        </div>
