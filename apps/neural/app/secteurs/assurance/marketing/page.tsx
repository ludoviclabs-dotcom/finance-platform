...
import {
  INSURANCE_MKT_AGENTS,
  INSURANCE_MKT_PROBLEMS,
  INSURANCE_MKT_SCENARIOS,
  INSURANCE_MKT_SERVICES,
  INSURANCE_MKT_SOURCES,
  INSURANCE_MKT_SUMMARY,
  INSURANCE_MKT_WORKBOOKS,
  type InsuranceMktAgentSlug,
} from "@/lib/data/insurance-marketing-catalog";

export const metadata: Metadata = {
  title:
    "Assurance / Marketing — 4 agents NEURAL : DDA, clarté, multi-canal, prévention | NEURAL",
  description:
    "Branche Assurance / Marketing : 4 agents portfolio, 5 workbooks Excel générés. Vulgarisation CGV (Flesch), audit DDA 12 points, déclinaison multi-canal (DSA comparateur), prévention RGPD art. 9.",
  openGraph: {
...
const AGENT_ICON = {
  "insur-simplifier": BookOpen,
  "dda-marketing-guard": ShieldCheck,
  "multi-channel-insur": Radio,
  "prevention-content": Sparkles,
} as const satisfies Record<InsuranceMktAgentSlug, typeof BookOpen>;
          ...
          <div className="mt-10 grid w-full min-w-0 gap-10 lg:grid-cols-[minmax(0,1.05fr)_minmax(360px,0.95fr)] lg:items-end">
            <div className="min-w-0 max-w-[320px] sm:max-w-none">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full border border-white/[0.12] bg-white/[0.04] px-3 py-1 font-mono text-[11px] text-white/62">
                  veille {INSURANCE_MKT_SUMMARY.sourceDate}
                </span>
              </div>
              ...
              <p className="mt-6 max-w-3xl text-lg leading-relaxed text-white/68">
                4 agents pour déverrouiller la chaîne marketing assurance :
                vulgariser les contrats, auditer les communications DDA,
                décliner par canal (avec DSA comparateurs) et générer des
                contenus prévention conformes RGPD. Le premier livrable est
                volontairement Excel — auditable, présentable, défendable.
              </p>
              <div className="mt-8 flex max-w-[320px] min-w-0 flex-col gap-3 sm:max-w-none sm:flex-row sm:flex-wrap">
                ...
                >
                  ...
                  {INSURANCE_MKT_WORKBOOKS.length} workbooks générés
                </a>
              </div>
            </div>
            ...
            <div className="grid max-w-[320px] min-w-0 gap-3 rounded-[28px] border border-white/10 bg-white/[0.045] p-4 shadow-2xl shadow-black/25 sm:max-w-none">
              <div className="grid gap-3 sm:grid-cols-2">
                {[
                  { label: "agents", value: INSURANCE_MKT_SUMMARY.agents },
                  { label: "workbooks", value: INSURANCE_MKT_SUMMARY.workbooks },
                  {
                    label: "règles DDA",
                    value: INSURANCE_MKT_SUMMARY.rules,
                  },
                  {
                    label: "scénarios",
                    value: INSURANCE_MKT_SUMMARY.scenarios,
                  },
                ].map((stat) => (
                  <div
                    key={stat.label}
              ...
              <div className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.08] p-4">
                ...
                <p className="mt-2 text-sm leading-relaxed text-amber-100/70">
                  Aucune communication n&apos;est diffusée sans verdict
                  défendable. L&apos;agent oriente — la décision finale reste
                  au compliance officer ou DPO.
                </p>
              </div>
            </div>
          </div>
            ...
            {[
              {
                ...
                value: INSURANCE_MKT_SOURCES.length,
                Icon: Database,
              },
              {
                label: "Workbooks Excel",
                value: INSURANCE_MKT_WORKBOOKS.length,
                Icon: FileSpreadsheet,
              },
              {
                label: "Scénarios démontrés",
                value: INSURANCE_MKT_SCENARIOS.length,
                Icon: Layers,
              },
              {
                label: "Services transverses",
                value: INSURANCE_MKT_SERVICES.length,
                Icon: Network,
              },
            ].map((item) => (
              <div
      ...
      <section className="border-b border-white/[0.08] px-6 py-14 md:px-12">
        ...
        >
          <SectionHeader
            ...
            title="4 problèmes marketing assurance, 4 agents contrôlables."
            lead="Chaque agent traite un problème opérationnel concret : illisibilité contractuelle, conformité DDA, cohérence multi-canal, prévention RGPD. Le périmètre est volontairement net."
          />
          <div className="mt-8 overflow-x-auto rounded-[24px] border border-white/10">
            <table className="w-full min-w-[720px] text-left text-sm">
              ...
              <tbody>
                {INSURANCE_MKT_PROBLEMS.map((row) => (
                  <tr
                    key={row.agent}
      ...
      >
        ...
        >
          <SectionHeader
            ...
            lead="Chaque agent dispose d'un fichier Excel auditable de 7 onglets : README, données d'entrée, audit/checklist, contenus optimisés, scoring, registre des sources. Disponible avant tout déploiement."
          />
          <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {INSURANCE_MKT_AGENTS.map((agent) => {
      ...
      <section className="border-b border-white/[0.08] px-6 py-14 md:px-12">
        ...
        >
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {INSURANCE_MKT_SCENARIOS.map((scenario) => {
      ...
      <section className="border-b border-white/[0.08] px-6 py-14 md:px-12">
        ...
        >
          <div>
            <SectionHeader
              eyebrow="Sourcebook"
              title="9 sources réglementaires actives."
              lead="Aucune sortie agent n'est validée sans mapping vers une de ces sources. La veille reste lisible — c'est utile pour un comité de pilotage et indispensable pour défendre la décision automatisée."
            />
            <div className="mt-6 overflow-hidden rounded-[22px] border border-white/10 bg-white/[0.035]">
              {INSURANCE_MKT_SOURCES.map((source) => (
                <div
                  key={source.id}
          ...
          <div className="grid gap-8 rounded-[28px] border border-white/10 bg-white/[0.045] p-6 md:p-8 lg:grid-cols-[0.9fr_1.1fr]">
            <div>
              ...
              <h2 className="mt-3 font-display text-3xl font-bold tracking-tight text-white md:text-4xl">
                {INSURANCE_MKT_WORKBOOKS.length} fichiers Excel prêts pour
                présentation.
              </h2>
              <p className="mt-4 text-sm leading-relaxed text-white/62">
                ...
                ou à un comité d&apos;investissement.
              </p>
              <div className="mt-6 flex flex-wrap gap-3">
                <Link
                  href="/contact?subject=Assurance%20Marketing%20-%20pilote"
            ...
            <div className="grid gap-2">
              {INSURANCE_MKT_WORKBOOKS.map((workbook) => (
                <div
                  key={workbook}
