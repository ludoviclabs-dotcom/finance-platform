...
### P7 — Suite de l'audit externe (juillet 2026) : BEGES suivi, campagnes fournisseurs, double matérialité, partenaires, SEO

- **T7.1** — Pages SEO réglementaires : `/bilan-carbone-beges` (seuils 500/250, scope 3 significatif obligatoire depuis le décret 2022-982, sanctions 50 k€/100 k€ loi Industrie verte, conditionnalité des aides publiques, FAQ + JSON-LD), `/cbam` (régime définitif 2026, seuil de minimis 50 t, certificats fév. 2027, déclaration 30/09/2027), `/scope3-fournisseurs` (collecte par lien sans compte, value chain cap), article blog « VSME devient VS » (projet d'acte délégué du 06/05/2026) + encart de mise à jour dans le guide CSRD & VSME. Sitemap à jour.
- **T7.2** — Suivi des dépôts BEGES : table `beges_filings` (migration 023, RLS), historique par exercice (date de dépôt, référence ADEME), échéance calculée (+4 ans, art. L229-25), rappels par paliers J-180/J-30/échéance via le centre de notifications (anti-spam par palier), e-mail optionnel. Endpoint cron `POST /beges/reminders/run` (`CRON_SERVICE_TOKEN` via `require_cron_or_analyst`) ; le cron Vercel quotidien orchestre désormais alertes + rappels BEGES + relances fournisseurs (limite Hobby : un seul cron). Section « Suivi des dépôts » sur `/beges`.
- **T7.3** — Campagnes de collecte fournisseurs : table `supplier_campaigns` (migration 024) + `campaign_id`/`viewed_at` sur les tokens et `review_status` sur les réponses ; import CSV de fournisseurs (stdlib, dédup par nom, anomalies lisibles) ; invitations tokenisées en masse ; suivi pending/viewed/completed (stamp SECURITY DEFINER `mark_supplier_token_viewed`) ; relances J-14/J-7/deadline (in-app + e-mails fournisseurs si `EMAIL_ENABLED`) ; **revue obligatoire des réponses avant intégration** (pattern gate FEC) avec détection d'anomalies pure (cohérence total/scopes, méthodologie, année) — accepter met à jour l'estimation GES du fournisseur. Page `/fournisseurs/campagnes`.
- **T7.4** — Double matérialité conforme ESRS 1 : **correctif méthodologique** — un enjeu est matériel si sa dimension impact OU financière atteint le seuil (l'ancien produit `sqrt(x×y)` ratait les impacts extrêmes à faible enjeu financier) ; détail par dimension, justification par enjeu (COALESCE en re-sauvegarde), mapping enjeu → standard ESRS à couvrir ; évaluations archivées immuables (migration 025) et export ZIP auditable (PDF deux dimensions + justifications, manifest `export_packages` domaine `materialite` → `/verify`). Panneau « Versions archivées » sur `/materialite`.
- **T7.5** — Programme partenaire experts-comptables (fondation) : page publique `/partenaires` (proposition honnête, sans grille tarifaire inventée — programme pilote sur candidature), table `partner_applications` (migration 026) + pipeline admin (`/partners/applications`), honeypot anti-bot, notification e-mail optionnelle. L'espace multi-dossiers reste `planifie` au registre (memberships multi-organisations = chantier de sécurité dédié).
- Registre `feature-status.json` mis à jour (nouvelle entrée `espace-cabinet`, descriptions campagnes/matérialité/BEGES, `derniere_maj` 2026-07-02) ; `.env.example` complété (SMTP T5.3, `CRON_SERVICE_TOKEN`, `BEGES_REMINDER_EMAIL`, `PARTNER_NOTIFY_EMAIL`, `FRONTEND_URL`).

### P5 — Pilotage & restitution

- **T5.1** — Courbe de coût d'abattement (MACC) : modèle `actions` (CapEx, réduction tCO2e/an, durée de vie, migration 020), coût marginal pur = CapEx / (réduction × durée), MACC triée par coût croissant (largeur = potentiel, aire = CapEx), export PDF déterministe. Les actions non chiffrées restent visibles.
- **T5.2** — Plan de transition opérationnel : statut proposée → engagée → réalisée avec journal append-only (`action_events`), vue Kanban, trajectoire projetée (réalisé/engagé/potentiel vs référence), export PDF « Plan de transition ». Page `/actions`.
- **T5.3** — Alertes stabilisées (in-app d'abord) : `alert_rules` + `alert_notifications` (migration 021, RLS), modes seuil absolu / variation % vs N-1 / donnée manquante, centre de notifications in-app persisté, e-mail **optionnel** via SMTP stdlib derrière `EMAIL_ENABLED` (zéro dépendance payante). Cœur d'évaluation pur. Page `/alerts` enrichie.
- **T5.4** — Adaptateurs d'import fichiers : parseurs AWS CCFT / GCP Carbon Footprint / Qonto CSV (purs, `csv` stdlib, sans OAuth ni SDK), screening Scope 3 via catalogue versionné, écran de revue obligatoire avant émission (gate, comme le FEC). Table `import_screenings` (migration 022, RLS). Page `/imports`.
- **T5.5** — Diff multi-exercices & mapping questionnaires : `diff_snapshots` pur (variations par poste, nouveaux/disparus, facteurs changés) entre les deux dernières versions ; export CSV « réponses prêtes » mappant les facts vers les questions types CDP allégé / EcoVadis environnement (catalogue versionné, ≥ 20 questions, référence du fact source, cellules sanitizées). Page `/diff`.
