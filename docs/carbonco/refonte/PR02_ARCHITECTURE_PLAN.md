...
> **Écart tranché par Ludo (2026-07-17, voir `PR02_DECISIONS.md`)** : `apply_one`/`apply_plan`/CLI `apply`/`AUTO_MIGRATE` sont **retirés** du périmètre PR-02B ci-dessous (initialement listés ici) et reportés à PR-02C — rien à appliquer aujourd'hui (`baseline` couvre déjà les 28 fichiers existants), et l'exécution SQL réelle est la pièce la plus risquée du runner. `acquire_lock` reste implémentée en PR-02B (nécessaire au bootstrap contrôlé et à `baseline`/`mark_manual_verified`), mais seulement dans sa forme simplifiée (verrou + budget de nouvelle tentative, sans le scénario multi-migrations d'`apply_plan`).

- **Périmètre** : bootstrap du ledger (§6), `acquire_lock` (§10, simplifié — voir écart ci-dessus), `baseline()`/`verify()`/`mark_manual_verified()`, CLI (`status`/`plan`/`verify`/`baseline`/`mark-manual-verified`, **pas** `apply`), `GET /health/schema`.
- **Dépendances** : PR-02A mergée (PR [#95](https://github.com/ludoviclabs-dotcom/finance-platform/pull/95)).
- **Tests** : concurrence simplifiée (2 connexions, pas la matrice complète), baseline sur base neuve/partielle/prod-simulée, checksum_mismatch, drift_detected.
- **Critères de sortie** : `baseline --dry-run` exécutable en toute sécurité contre une base de test (jamais la prod) donne un résultat cohérent avec l'inventaire du §3, y compris la baseline `version='000'` du DDL inline historique (représentation synthétique, absente de `discover_migrations()`/`build_plan()` qui restent inchangés depuis PR-02A — voir `migration_runner.py::_synthetic_000_file`).
