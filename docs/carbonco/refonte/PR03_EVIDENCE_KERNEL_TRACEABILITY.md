...
- **Tests PostgreSQL/RLS/trigger désormais VERTS en CI réelle** (voir §15) — après 3 allers-retours CI qui ont révélé et corrigé de vrais défauts. La migration 028 s'applique proprement contre `postgres:16`, ses triggers/CHECK/contraintes d'unicité et l'isolation tenant sont exercés à chaque run. La limite restante : l'isolation tenant est validée en CI via un prédicat applicatif de défense-en-profondeur (le rôle CI `postgres` est superuser et bypasse la RLS) ; l'effet de la RLS elle-même sous rôle non-privilégié reste vérifié structurellement (`_probe_028`) + en staging sous `carbonco_app`.
...
- **Aucune migration exécutée contre Neon ou toute base réelle.** Localement : `py_compile`, `pytest` (DB-gated skippés faute de docker), `ruff`. En CI : migration 028 appliquée contre un conteneur `postgres:16` **jetable** (jamais Neon) — cf. §15.
...
## 15. Durcissement piloté par la CI (3 allers-retours, commits post-ouverture PR)

Comme à chaque tranche de ce chantier (pas de PostgreSQL local, cf. PR-02A/B/C), la première preuve réelle est venue de la CI. Le job `migration-tests` (conteneur `postgres:16`) a révélé et fait corriger trois choses successives — la migration 028 elle-même s'est toujours appliquée proprement (triggers, CHECK, unicité confirmés par les logs du conteneur) :

1. **Sonde 028 vs « base complète » (commit `647a1f6`).** Le premier run échouait sur `_probe_028` et sur les tests PR-02B qui baselinent toutes les versions : le fixture `build_full_db` s'arrêtait à 027 alors que `discover_migrations()` retourne désormais 028 — la « base complète » ne contenait donc pas la version 028 sondée. Corrigé : `build_full_db` applique jusqu'à 028 (aligné sur `discover_migrations`), et les 4 `written_count` 29→30. **Découverte au passage** : les tests DB-gated de PR-03 (`test_intelligence_*`) n'étaient exécutés par AUCUN job (le job `tests` tourne en mode /tmp, ils y skippent tous) — ajoutés au job `migration-tests`, seul à disposer d'un vrai PostgreSQL. Sans cet ajout, la migration 028 et sa RLS n'auraient jamais été exercées en CI.

2. **Teardown des tables immuables (commit `9bcbd7a`).** Le nettoyage des fixtures (`DELETE FROM source_releases/observations`) était refusé par les triggers d'immutabilité que la migration installe (append-only / frozen) — 3 erreurs de teardown. Le teardown pose désormais `session_replication_role = replica` (désactive les triggers utilisateur le temps du cleanup ; rôle superuser requis, disponible en CI).

3. **Isolation tenant sous connexion superuser (commit `9bcbd7a`).** Le PostgreSQL de CI se connecte en `postgres` (**superuser**), qui **bypasse toute RLS**, `FORCE` compris — donc 8 tests d'isolation échouaient (tenant A voyait tenant B). C'est précisément pourquoi le `test_rls_isolation.py` historique du dépôt n'est **pas** en CI. Résolution retenue : ajouter un **prédicat de périmètre explicite dans chaque requête des services** (`(company_id = tenant OR IS NULL)` en lecture, `company_id = tenant` en écriture) — même défense-en-profondeur anti-IDOR que `evidence_service.get_evidence_file`. L'isolation tient désormais **quel que soit le rôle DB** (superuser inclus), et la RLS reste la garantie primaire en prod sous `carbonco_app`. Le test « tenant ne peut pas écrire global » (pure policy RLS, invérifiable sous superuser) a été reformulé en garantie de service (`create_source` force toujours `company_id = tenant`).

**Résultat CI (commit `9bcbd7a`, run `29627024274`)** : `migration-tests` ✅, `tests` ✅, `validate` ✅, `security-audit` ✅, `gitleaks` ✅. Seul `Vercel – neural` est rouge — flakiness d'infra connue et sans rapport (monorepo : chaque push redéploie tous les projets ; `neural` n'a aucun fichier touché par cette PR).

**Leçon (rappel du chantier)** : « vert en local » ne prouve rien pour du code DB-gated sans PostgreSQL local ; et ajouter un test DB-gated sans l'inscrire dans le job `migration-tests` le rend silencieusement inerte en CI.
