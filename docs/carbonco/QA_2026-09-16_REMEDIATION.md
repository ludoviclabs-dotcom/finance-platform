...
| Suite e2e historique (`e2e.yml`) | Annulée à chaque exécution depuis juillet (délai de 20 min), faute de secret `E2E_API_URL`. **Option 2 appliquée par PR dédiée** : projet Playwright `sans-api` (61 tests) joué à chaque push, projet `avec-api` (77 tests) seulement si `E2E_API_URL` et `E2E_USER_PASSWORD` existent (guide, étape 10). Au passage, `GET /api/auth/demo` en 401 libère son corps de réponse : la requête restait ouverte et bloquait `networkidle` (13-demo, 01-auth). | Playwright local sur build de production : `sans-api` 61/61, 3 passes sans nouvel essai ; `tests/demo-session-request.test.ts` |
