...
- **Limite P03 constatée, non corrigée dans cette vague.** `derive_observations()` fixe `period_start == period_end` et ne recopie pas les métadonnées du draft : une période trimestrielle ne peut pas être portée telle quelle par le read model P02. Contournement retenu sans modifier P03 — le trimestre est encodé dans le `metric_code` et la date d'observation est le premier jour du trimestre. À arbitrer en P10 : propager la période et les métadonnées dans `derive`, ou officialiser la convention `metric_code`.
